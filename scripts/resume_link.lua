...
-- 快捷方式默认名为“!继续之前播放”，使用 ! 前缀使其在常规名称正序排列时靠前。
...
local function create_shortcut(media_path, position, reason)
  ...
  local arguments = string.format(
    '--config-dir="%s" --no-resume-playback ' .. '--script-opts=resume_link-resume_position=%.3f -- "%s"',
    config_dir,
    position,
    media_path
  )
...
local function on_shutdown()
  ...
  if
      last_saved_reason == "end-file"
      and last_saved_path == current_path
      and current_duration > 0
      and last_position >= current_duration - 1
  then
    return
  end
