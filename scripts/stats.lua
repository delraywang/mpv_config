...
-- 基于 mpv 官方 stats.lua，增加中文统计标签；统计页面和按键绑定保持兼容。

local mp = require 'mp'
local utils = require 'mp.utils'
local input = require 'mp.input'

-- 根据 uosc 的首选语言加载对应的统计信息文本。
...
local function get_first_language(value)
    local language = value and value:match("^%s*([^,%s]+)")
...
local function find_language(content, pattern)
    if not content then
        return nil
    end
    ...
    for line in content:gmatch("[^\r\n]+") do
        if not line:match("^%s*[#;]") then
            ...
            if value then
                return get_first_language(value)
            end
        end
    end
...
local function get_configured_language(config_dir)
    local uosc_config = utils.join_path(
        utils.join_path(config_dir, "script-opts"),
        "uosc.conf"
    )
    local language = find_language(
        read_file(uosc_config),
        "^%s*languages%s*=%s*(.+)%s*$"
    )
    if language then
        return language
    ...
    -- 兼容集中式 script-opts.conf 写法。
    local script_opts = utils.join_path(config_dir, "script-opts.conf")
    return find_language(
        read_file(script_opts),
        "^%s*script%-opts%-append%s*=%s*uosc%-languages%s*=%s*(.+)%s*$"
    )
end

local function load_locale()
    local empty_locale = { text = {}, perf = {}, perf_prefixes = {} }
    ...
    local language = get_configured_language(config_dir)
    if not language then
        return empty_locale
    end

    local locale_dir = utils.join_path(config_dir, "scripts")
    locale_dir = utils.join_path(locale_dir, "stats_locales")
    local candidates = { language }
    local base_language = language:match("^([^%-]+)")
    if base_language and base_language ~= language then
        candidates[#candidates + 1] = base_language
    end

    for _, candidate in ipairs(candidates) do
        local locale_path = utils.join_path(locale_dir, candidate .. ".lua")
        local ok, locale = pcall(dofile, locale_path)
...
local text_translations = locale.text or {}
local perf_translations = locale.perf or {}
local perf_prefixes = locale.perf_prefixes or {}
local label_separator = locale.label_separator or ":"

-- 没有翻译时原样返回；有翻译时先精确匹配，再进行较长文本替换。
local function translate_text(text)
    if type(text) ~= "string" then
        return text
    end

    local exact = text_translations[text]
    if exact ~= nil then
        return exact
    end

    local keys = {}
    for source in pairs(text_translations) do
        if #source >= 4 then
            keys[#keys + 1] = source
        end
    end
    ...
    for _, source in ipairs(keys) do
        if text:find(source, 1, true) then
            text = text:gsub(source, function() return text_translations[source] end, 1)
        end
    end
    return text
end
...
local function append(s, str, attr)
    ...
    str = translate_text(str)
    if attr.prefix then
        attr.prefix = translate_text(attr.prefix)
    end
    if attr.suffix then
        attr.suffix = translate_text(attr.suffix)
    end
...
local function scroll_hint(search)
    ...
    hint = hint .. ")"
    if not o.use_ass then return translate_text(" " .. hint) end
    return translate_text(format(" {\\fs%s}%s{\\fs%s}", font_size * 0.66, hint, font_size))
end
...
local function cmd_subject(cmd)
    ...
    if sname and sname ~= "" then
        return translate_text("script: " .. sname)
    end
    ...
    return translate_text(subw:len() > 1 and subw or "[unknown]")
end
...
local function append_general_perfdata(s)
    for i, data in ipairs(mp.get_property_native("perf-info") or {}) do
        local display_name = perf_translations[data.name] or data.name
        if display_name == data.name then
            for prefix, translated_prefix in pairs(perf_prefixes) do
                if data.name:sub(1, #prefix) == prefix then
                    display_name = translated_prefix .. data.name:sub(#prefix + 1)
                    break
                end
            end
        end
        display_name = translate_text(display_name)
        append(s, data.text or data.value, {
            prefix="["..tostring(i).."] "..display_name..label_separator
        })
...
for _, page in pairs(pages) do
    page.desc = translate_text(page.desc)
end
