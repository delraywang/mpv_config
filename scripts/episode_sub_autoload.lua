...
local NOISE_TOKENS = {
    aac = true, ac3 = true, atmos = true, av1 = true, bdrip = true, bluray = true,
    brrip = true, cam = true, ddp = true, dv = true, dts = true, dtsma = true,
    dual = true, dub = true, extended = true, hevc = true, hdr = true, hdr10 = true,
    flac = true, h264 = true, h265 = true, hdtv = true, opus = true, proper = true,
    repack = true, remux = true, truehd = true,
    rip = true, uhd = true, web = true, webdl = true, webmux = true, webrip = true,
    x264 = true, x265 = true,
}

local LANGUAGE_ALIASES = {
    zh = "zh", chs = "zh", cht = "zh", chi = "zh", zho = "zh", sc = "zh",
    cn = "zh", tw = "zh", hk = "zh",
...
local function is_resolution_token(token)
    return token:match("^%d+[pi]$") ~= nil
        or token:match("^%d%d%d+%dx%d%d%d+$") ~= nil
end
...
local function is_episode_token(token)
    return token:match("^s%d+e%d+$") ~= nil
        ...
        or token == "episode" or token == "ep" or token == "e" or token == "第" or token == "集"
end
...
local function build_match_key(stem)
    ...
    for _, token in ipairs(tokenized_name(stem)) do
        ...
        if not NOISE_TOKENS[token] and not is_language_token(token)
            and not is_resolution and not is_bit_depth then
            kept[#kept + 1] = token
        end
    end
...
local function build_title_tokens(stem)
    ...
    for _, token in ipairs(tokenized_name(stem)) do
        ...
        local is_resolution = is_resolution_token(token)
        local is_bit_depth = token:match("^%d+bit$") ~= nil
        if not NOISE_TOKENS[token] and not is_language_token(token) and not is_episode_token(token)
            and not is_number and not is_resolution and not is_bit_depth and not seen[token] then
            seen[token] = true
...
local function parse_episode(stem)
    ...
    first = spaced:match("%f[%a]episode%s*(%d+)")
        or spaced:match("%f[%a]ep%s*(%d+)")
        or spaced:match("%f[%a]e%s*(%d+)")
    if first then
        return make_episode(nil, first)
    end
...
local function episodes_match(video_episode, subtitle_episode)
    ...
    if video_episode.season and subtitle_episode.season
        and video_episode.season ~= subtitle_episode.season then
        return nil
    end
    if math.max(video_episode.first, subtitle_episode.first)
        > math.min(video_episode.last, subtitle_episode.last) then
        return nil
    end
...
local function detect_languages(stem)
    local text = tostring(stem or ""):lower()
    local result, seen = {}, {}
    if text:find("简中", 1, true) or text:find("简体", 1, true)
        or text:find("繁中", 1, true) or text:find("繁体", 1, true)
        or text:find("中文字幕", 1, true) then
    ...
    for _, filename in ipairs(files) do
        ...
        if is_supported_subtitle(filename, extensions) and not paired_idx then
            ...
            if base then
                ...
                local score = base + similarity + language_bonus(languages, preferred)
                    + (FORMAT_BONUS[extension] or 0)
...
local function process_current_file()
    ...
    if next_best and best.score - next_best.score < minimum_gap then
        ...
        mp.msg.info(string.format(
            "字幕候选分差不足: %s (%d) / %s (%d)",
            best.filename, best.score, next_best.filename, next_best.score
        ))
        return
    end

    local ok, command_result, command_error = pcall(
        mp.commandv, "sub-add", best.path, "select", best.filename
    )
    if not ok or command_result == nil then
        show_message("加载字幕失败")
        mp.msg.error("无法加载字幕 " .. best.path .. ": "
            .. tostring(ok and command_error or command_result))
        return
    end
