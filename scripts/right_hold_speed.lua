...
local function show_temporary_speed_message()
    if not temporary_speed then return end
    ...
    speed_message_overlay.data = string.format(
        "{\\an9\\pos(%d,%d)}当前倍速%s",
        width - 30,
        30,
        speed_text
...
local function handle_temporary_speed_key(event, delta)
    if event.event == "down" or event.event == "repeat" then
        if not key_down or not holding or not temporary_speed then return end
...
rearm_right_binding = function()
    if not key_down or not right_binding_canceled then return end
